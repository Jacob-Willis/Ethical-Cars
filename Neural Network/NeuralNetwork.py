...
class optimiser_SGD:
    ...
    # Initialize optimiser - set settings,
...
class Model:
    ...
    # Saves the model
    def save(self, path):
        # Make a depp copy of current model instance
        ...
        # For each layer removev inputs, output and dinputs properties
        for layer in model.layers:
            for property in ['inputs', 'output', 'dinputs',
                             'dweights', 'dbiases']:
                layer.__dict__.pop(property, None)
        
        # Open a file in the binary read-write mode and save the model
        with open(path, 'wb') as f:
            pickle.dump(model, f)
    ...
    def predict(self, X, *, batch_size=None):
        ...
        for step in range(prediction_steps):
            # If batch is not set -
            # train using one step and a full dataset
            if batch_size is None:
                batch_X = X
